...
registerMooseObject("otterApp", CLCalc2);
...
CLCalc2::CLCalc2(const InputParameters & parameters)
...
CLCalc2::computeValue()
{
    ...
    RealVectorValue term2 = (1.0 - _d[_qp]) * (1.0 - _d[_qp]) * _D_L * _grad_C_L[_qp]; // ;
    // Real term3 = -((1.0 - _d[_qp]) * (1.0 - _d[_qp]) * _D_L * _C_L[_qp] * _V_H * _grad_hidrostatic_stress[_qp]) / (_R * _Theta); // * _grad_hidrostatic_stress[_qp]
